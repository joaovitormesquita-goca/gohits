...
// Upload image as base64 bytes via multipart form — required for adimages endpoint
async function metaUploadImage(imageUrl: string): Promise<string> {
  // Download server-side so signed/private Supabase Storage URLs are resolved before Meta sees them
  const imageRes = await fetch(imageUrl)
  if (!imageRes.ok) throw new Error(`Failed to download image from ${imageUrl}: ${imageRes.status}`)
  ...
  const url = `${META_API_BASE}/act_${adAccountId().replace('act_', '')}/adimages`
  const res = await fetch(url, { method: 'POST', body: form })
  const data = await res.json()
  if (!res.ok || data.error) throw new Error(data.error?.message ?? 'Image upload to Meta failed')
...
async function createAdCreative(imageUrl: string, title: string, body: string): Promise<string> {
  const imageHash = await metaUploadImage(imageUrl)
